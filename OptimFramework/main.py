...
def main_loop(constraint_name: str,
    ...
    Returns
    -------
    None - appends to file found at data_path
    ...
    while True:
        ...
        print("Accuracy of the Constraintraint fit: Y, Y_Est, Diff")
        ...
        if algo_type.lower() == 'newton':
            ...
            xopt, *_ = optim.sampling_optimize_tt(GC_Constraint, GC_Obj, constraint_val, 
                                            ...
                                            num_optims=num_optims, to_select = 100, 
                                            xopt = xopt)
            # inputs 1x num_vars: optimal
            # constraint: scalar value of constraintraints at optimal
...
def toy_func(x):
    ...
    constraintraint_func = np.nansum(x, axis=-1)
    return x, constraintraint_func, obj_func
...
def init_toy_problem_ros(num_samples = 10000, num_vars = 2):
    #cols = []
    #data = pd.DataFrame(data = [], columns = cols)
    #for i in range(30):
    #    cols.append("x"+str(i))
