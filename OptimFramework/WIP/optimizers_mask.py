...
def get_utility(X, GC_Constraint, GC_Obj, constraint_val, max_order_obj, 
                max_order_constraint, upper_bound, lower_bound, upper_factor, 
    ...
    lower_bound_val = np.exp( (lower_bound-X) / lower_factor ).sum(axis = 1)
    upper_bound_val = np.exp( (X-upper_bound)/ upper_factor ).sum(axis = 1)

    #print("bound_error =", upper_bound+lower_bound)
    #print("constraint_error =", constraint_penalty)
    #print("Obj_error", obj)
    utility = 1.0*obj + 1.0*constraint_penalty + 1e2 * (upper_bound_val + lower_bound_val) + 0e1 * np.nansum((X - 0.5)**2., axis=1)

    return utility, obj, constraint_penalty, upper_bound_val, lower_bound_val


def linesearch(X, dx, GC_Constraint, GC_Obj, constraint_val, 
               max_order_obj, max_order_constraint, upper_bound, 
               lower_bound, upper_factor, lower_factor, constraint_factor):
    ...
    baseline_util, _, _, _, _ = get_utility(X, GC_Constraint, GC_Obj, constraint_val, max_order_obj = max_order_obj, 
                                            max_order_constraint = max_order_constraint, upper_bound = upper_bound, 
                                            lower_bound = lower_bound, upper_factor = upper_factor, 
    ...
    for i in np.arange(-2.0, 2.0, 0.15):
        ...
        new_X = (X + i * dx).clip(lower_bound,upper_bound)
...
def get_bounds_deriv(X, upper_bound, lower_bound, upper_factor, lower_factor):
    ...
    lower_bound_jac = - np.exp((lower_bound-X)/lower_factor) / lower_factor
    upper_bound_jac = np.exp( (X-upper_bound)/upper_factor ) / upper_factor
    ...
    for i in range(num_points):
        lower_bound_hess[i] = np.diag(np.exp((lower_bound-X[i])/lower_factor) / lower_factor**2.)
        upper_bound_hess[i] = np.diag(np.exp((X[i]-upper_bound)/upper_factor) / upper_factor**2.)
...
def gradient_optimize_tt(GC_Constraint, GC_Obj, n = 1, constraint_val = 3e6, max_order_obj = 4, 
                         max_order_constraint=4, upper_bound = 1, lower_bound = 0, upper_factor = 0.01, lower_factor = 0.01,
    ...
    for i in range(200):
        ...
        constraint_jac, constraint_hess = get_constraint_deriv(GC_Constraint, X, constraint_val, 
                                                               max_order_constraint = max_order_constraint, 
                                                               constraint_factor = constraint_factor)
        boundary_jac, boundary_hess = get_bounds_deriv(X, upper_bound=upper_bound, lower_bound=lower_bound, 
        ...
        utility_x, obj, constraint_penalty, upper_bound, lower_bound = get_utility(X, GC_Constraint, 
                           ...
                           max_order_constraint = max_order_constraint, upper_bound=upper_bound, lower_bound=lower_bound, 
                           upper_factor = upper_factor, lower_factor = lower_factor, 
                           constraint_factor = constraint_factor)
...
def sampling_optimize_tt(GC_Constraint, GC_Obj, constraint_val, max_order_obj = 4, max_order_constraint = 4, 
                         upper_bound = 1, lower_bound = 0, xopt = None, to_select = 100):
    Xselected = None
    #X = np.random.rand(int(100000/num_iters), len(GC_Constraint))
    X = np.random.rand(100000, len(GC_Constraint))*upper_bound
    if xopt is not None:
        M = xopt.shape[0]
    ...
    for iloop in range(50):  
        ...
        for j in range(len(Xselected)):
            X = (0.2 * np.random.rand(to_select,1)) * (np.random.rand(to_select, len(GC_Constraint))-0.5) * np.sqrt(upper_bound)
            X[0] = Xselected[j]
            X[1:] = X[1:] + Xselected[j]
            #print(X, upper_bound, lower_bound)
            X = X.clip(lower_bound,upper_bound)
            #print(X, upper_bound, lower_bound)
            # compute objective given set of points
